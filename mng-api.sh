...
PROJECT_NAME='vo_bot'
SERVER_NAME='Azure'
PROJECT_PATH='/var/www/vo_bot_django/'
...
function make_migrations() {
    docker exec -it ${API_CONTAINER_NAME} /app/manage.py makemigrations
}

function showmigrations() {
    echo -e "\n ... showmigrations ... \n"
    docker exec -it ${API_CONTAINER_NAME} /app/manage.py showmigrations $1
}

function migrate() {
    echo -e "\n ... migrate db ... \n"
    docker exec -t $1 /app/manage.py migrate $2 $3
}
...
function shell() {
    docker exec -it ${API_CONTAINER_NAME} /app/manage.py shell
}
...
function collectstatic() {
    ...
    docker exec -i $1 /app/manage.py collectstatic --noinput
}

function create_admin_user() {
    docker exec -it ${API_CONTAINER_NAME} /app/manage.py shell -c "from django.contrib.auth.models import User; User.objects.create_superuser('${1:-admin}', '', '${2:-test1234}')"
}

function issue_https_certificate() {
    sudo certbot --nginx certonly -d ai-playground-api.vibesonly.com
